...
gcc = package('gcc')
eglibc = package('eglibc')
linux = package('linux')
specs = Commands.new(load_misc('specs'), @exec, @log)

[ binutils, gcc, eglibc, linux, specs ].each { |r| r.set(cfg) }

linux.build('sysroot headers')
binutils.build
gcc.build('static compiler')
eglibc.build('sysroot eglibc')
gcc.build('full compiler')
linux.build('temporary tool headers')
eglibc.build('temporary tool eglibc')
specs.build

[ binutils, gcc, eglibc, linux ].each { |r| r.cleanup }
